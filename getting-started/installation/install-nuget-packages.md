...
page_title: Install Telerik Document Processing Libraries with NuGet Packages
description: Install Telerik Document Processing from NuGet.org, a local feed, or the Telerik NuGet server. Includes explicit PackageReference examples, guidance for Blazor and CI/CD environments, and steps to avoid package-source confusion.
...
Add the package references in the application project that generates, imports, or exports documents. Install `Telerik.Licensing` in the same project, keep all Telerik Document Processing packages on the same release version, and do not mix `Telerik.Documents.*` with `Telerik.Windows.Documents.*` in the same application.
...
### Install from NuGet.org with the .NET CLI
...
### Install from NuGet.org with Package Manager Console
...
### Blazor Web App, Blazor Server, and ASP.NET Core
...
## Install from a Local NuGet Feed
...
## Install from the Telerik NuGet Server
...
### Add the Telerik NuGet Server to Visual Studio
...
### Reset Stored Telerik Feed Credentials
