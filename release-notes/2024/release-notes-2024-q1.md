...
* A `NullReferenceException` is thrown when getting the hash code of a CFF font's `UnderlineThickness`.
...
* A `NullReferenceException` is thrown on export because the value of the `TransformMethod` property is not obtained.
...
* Import-export of a specific document with `Type1Font` mangles text.
...
* Changed the `GetCryptoProvider()` method signature. It now accepts `EncryptionSettingsBase` instead of `EncryptionSettings`.
