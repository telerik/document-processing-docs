...
* Starting with 2025 Q1, all users of the Telerik and Kendo UI components and tools need to apply a valid license key file to both new and existing projects. For details, see the [Licensing Documentation]({%slug setting-up-license-key%}).
...
* A `NullReferenceException` is thrown when importing a document and enumerating the pages.
...
* A `NullReferenceException` is thrown when exporting a workbook with no sheets. A message indicating that a workbook must contain at least one visible sheet is now displayed.
