...
>note Telerik Document Processing Libraries have a version which may differ from the version of the other Telerik products used in the particular case. The changes are synced between the components that have a dependency on Telerik Document Processing and while the referenced version are from the same release, they should work as expected regardless of the version distinction. 
